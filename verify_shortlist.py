...
- recruitable: "Yes" for FINAL-YEAR PhD students, recent grads, academics, startup folks, and industry ICs at ANY company including Meta, DeepMind, Google, NVIDIA, Microsoft (poaching from big labs is expected); "Maybe" only when something concrete argues against a move (just started a new role, visa tie, deep vesting); "Unlikely - at X" ONLY for OpenAI and Anthropic, tenured faculty, founders running their own company, or "Unlikely - early PhD (Nth yr)" for 1st-3rd year PhD students (too early to hire). Add a short reason ("Yes - just switched labs").
